class DeadlyPoison extends Ability {
    ...
    getTooltip() {
        return "Coats your weapons with a Lethal Poison that lasts for 1 hour. Each strike has a 50% chance to poison the enemy for "+((player.stats.primary * this.effectValue.spellPowerDot) * (1 + (player.stats.vers / 100)) * (1 + (player.stats.haste / 100))).toFixed(0)+" Nature damage over 12 sec. Subsequent poison applications will instantly deal "+((player.stats.primary * this.spellPower) * (1 + (player.stats.vers / 100))).toFixed(0)+" Nature damage."
    }
